          ...
          <div className="px-2">
            {conversations.map((conv) => (
              <Link
                ...
                className={`block p-3 mb-2 rounded-md hover:bg-gray-100 ${
                  conv.id === currentConversation?.id ? 'bg-blue-50 border-l-4 border-blue-600' : ''
                }`}
              >
                <div className="font-medium text-sm">{conv.projectTitle || 'Sans titre'}</div>
          ...
          {currentConversation ? (
            <>
              ...
              <div className="bg-white border-b px-6 py-4 flex justify-between items-center">
                <div>
                  <h2 className="text-lg font-semibold">{currentConversation.projectTitle}</h2>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {matchingLoading ? 'Matching...' : 'Trouver des Professionnels'}
                  </button>
                  {matches.length > 0 && (
                    <button
                    ...
                    >
                      {showMatches ? 'Masquer' : 'Voir'} Matches ({matches.length})
                    </button>
                  )}
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                {messages.length === 0 ? (
                  <div className="text-center text-gray-500 mt-12">
                    <p className="text-lg">Commencez votre brainstorming!</p>
                    <p className="text-sm mt-2">Décrivez votre projet et je vous aiderai à le développer.</p>
                  </div>
                ) : (
                ...
                {loading && (
                  <div className="flex justify-start">
                    <div className="max-w-2xl px-4 py-3 rounded-lg bg-white border border-gray-200">
                      <p className="text-gray-500">L'IA réfléchit...</p>
                    </div>
                  </div>
                )}
        ...
        {showMatches && matches.length > 0 && (
          <div className="w-96 bg-white border-l overflow-y-auto">
            <div className="p-4 border-b">
              <h3 className="font-semibold text-lg">Professionnels Matchés</h3>
              ...
              {matches.map((match: any) => (
                <div key={match.id} className="p-4">
                  ...
                  {match.professional.hourlyRate && (
                    <p className="text-sm text-gray-600">
                      {match.professional.hourlyRate}€/heure
                    </p>
                  )}
