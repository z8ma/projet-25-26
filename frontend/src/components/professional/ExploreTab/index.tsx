...
import { PortfolioViewModal } from '../PortfolioTab/PortfolioViewModal';
...
export function ExploreTab() {
  ...
  // Scroll animations
  const headerAnimation = useScrollAnimation([subTab]);
  const contentAnimation = useScrollAnimation([subTab]);

  // Load professions list once
  useEffect(() => {
    loadProfessions();
  }, []);
  ...
  // Load portfolios when filters change
  useEffect(() => {
    if (subTab === 'projects') {
      loadPortfolios();
    }
  ...
  const loadPortfolios = async () => {
    ...
    try {
      ...
      // Load liked portfolios if user is a professional
      if (user?.role === 'PROFESSIONAL' && loadedPortfolios.length > 0) {
        try {
  ...
  const getPrimaryProfession = (professional: Professional | PortfolioItem['professional']) => {
  ...
  const getPortfolioImage = (portfolio: PortfolioItem) => {
    // Priority: custom cover image -> first IMAGE type media -> first media thumbnail -> first media url
    return (
      portfolio.imageUrl ||
    ...
    try {
      if (likedPortfolios.has(portfolioId)) {
        ...
        setPortfolios(prev => prev.map(p =>
          p.id === portfolioId ? { ...p, _count: { likes: p._count.likes - 1 } } : p
        ));
      } else {
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-gray-500 mt-0.5 text-sm">
              Découvrez des projets et des professionnels créatifs
            </p>
          </div>
        </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-3.5 bg-white border border-gray-200 rounded-2xl text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all"
                />
                {projSearch && (
                  <button
                ...
                {portfolios.map((portfolio) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Like button (for professionals only) */}
                      {user?.role === 'PROFESSIONAL' && (
                        ...
                        >
                          <svg
                            className={`w-3.5 h-3.5 transition-colors ${likedPortfolios.has(portfolio.id) ? 'text-red-500 fill-red-500' : 'text-gray-400'}`}
                      ...
                      {portfolio._count.likes > 0 && (
                        <div className={`absolute ${user?.role === 'PROFESSIONAL' ? 'top-10 right-2' : 'top-2 right-2'} flex items-center gap-0.5 px-1.5 py-0.5 bg-black/50 backdrop-blur-sm text-white text-[10px] rounded-full`}>
                      ...
                      <div className="p-2.5">
                        ...
                        <div className="flex items-center gap-1.5 mt-1.5">
                          {portfolio.professional.profilePictureUrl ? (
                            ...
                            <div className="w-5 h-5 bg-gradient-to-br from-purple-400 to-purple-600 rounded-full flex items-center justify-center text-white text-[8px] font-bold flex-shrink-0">
            ...
            {proLoading ? (
              ...
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {professionals
                  ...
                  <div
                    key={professional.id}
                    className="relative bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden cursor-pointer hover:shadow-lg hover:-translate-y-1 transition-all duration-200"
                  ...
                  >
                    ...
                    <div className="flex justify-center -mt-8 relative z-10">
                      {professional.profilePictureUrl ? (
                        ...
                        <div className="w-16 h-16 bg-gradient-to-br from-purple-400 to-purple-600 rounded-full flex items-center justify-center text-white font-bold text-lg border-4 border-white shadow-md">
